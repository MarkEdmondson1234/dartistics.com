...
In R's mind, both `my_var` and `myVector` are actually vectors. `my_var` is simply a vector of length `1`, while `myVector` is a vector of length `4`. Let's illustrate that and pick up a few more basics of the R/RStudio environment.
...
Type `typeof(myVector)`. Note that it is the identical _type_ as `my_var`.
```{r}
typeof(myVector)
```

Now, type `length(my_var)` and then `length(myVector)`. Note that this is where the two objects differ.
```{r echo=TRUE}
length(my_var)
length(myVector)
```
...
Type in `ga_data` in the console. This will simply quickly scroll all of the contents of the data frame.
...
Don't judge a platform by your first visualisation. Even with the basics of `plot()`, this chart can be a lot better, but we're actually going to primarily focus on `ggplot2()` when it comes to visualisations. So, let's call this initial exercise a wrap! Congratulations! You've loaded, manipulated, and plotted data with R!