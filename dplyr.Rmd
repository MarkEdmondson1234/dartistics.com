...
## dplyr
The `dplyr()` package is one of those packages that, consistently, newcomers to R do not know about and who then get confused by some (one) aspect of it...but experienced R users seldom right a script without using it.

## The dplyr Functions
`dplyr` has just a handful of functions, all of which are geared towards doing basic manipulation of data sets with fairly straightforward functions. We're not going to go into all of the details of using these functions, as there are plenty of write-ups of that (like [this one](https://cran.rstudio.com/web/packages/dplyr/vignettes/introduction.html)). But, we will at least provide a brief description of the functions and, at a high level, what they do:
...
* `mutate()` -- used to add new columns that are based on calculations on data in other columns
* `summarise()` -- used to perform summary calculations (mean, max, etc.) on a set of data
* `sample_n()` -- used to return a sample from the data set

A _key_ aspect of all of these functions is that _the first argument is always the data set. There is some magic as to how this works in conjunction with the pipe (`%>%`), which we're going to tackle next.
...
Let's consider our web analytics data (`webData`) that has sessions by date, device category, and channel:

```{r echo=FALSE}
webData <- read.csv("data/gadata_example_2.csv", stringsAsFactors = FALSE)
webData <- webData[,c("date","deviceCategory","channelGrouping","sessions")]
colnames(webData) <- c("Date","Device","Channel","Sessions")
```
...
display_traffic <- webData[webData$Channel == "Display",]
...
final_data <- webData[(webData$Channel == "Display" & webData$Device == "mobile" 
                       & webData$Sessions > 2000),]
...
```{r}
round(mean(webData[(webData$Channel == "Display" & webData$Device == "mobile" 
                       & webData$Sessions > 2000),]$Sessions),0)
...
webData %>% filter(Channel == "Display") %>%
  filter(Device == "mobile") %>%
  filter(Sessions > 2000) %>%
...
1. Take the web data and filter it to just the "Display" rows.
2. Then, filter _that_ to just be the "mobile" rows.
3. Then, filter _that_ to just be rows where "Sessions" are greater than 2 000.
4. Summarise the resulting data subset by taking the mean of "Sessions."
5. Round the mean to the nearest integer.
...
It returns the same result, and you would never really right it this way. But, when using pipes with functions that are not `dplyr` functions, sometimes, you don't want the result of a function to be passed into the first argument. So, the `.` comes in handy!
