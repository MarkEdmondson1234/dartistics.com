...
The most universal data format is `.csv` (comma seperated variables). 
...
When you do, you see that `read.csv` is a derived function from the more general `read.table` that can read other formats such as tab seperated (`.tsv`).  
...
A helpful argument to also include is `stringsAsFactors = FALSE` - a classic R quirk, it stops future problems where R treats strings are factors that may cause unexpected results.
