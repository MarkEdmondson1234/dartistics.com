...
We're going to skip the step of actually pulling data directly using an API (for now), so just download the file at [http://bit.ly/r_web_data](http://bit.ly/r_webData). Upload that file to your working directory, and then load that file into your R environment as an object (data frame) named `web_data`:
