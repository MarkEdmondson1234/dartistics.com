...
tukey_results <- lapply(all_groups, my_anova_tukey, metric = "GoalCompletions", dim = "channelGrouping")
```

This is a big list. (each channel compared with one another, per group).

We look at just one channel for Group 1:

```{r}
# tukey_results$group_one$`channelGrouping:deviceCategory`
tukey_results$group_one$channelGrouping
```

### Multi way ANOVA

* Hoa the group means for first factor are equal
* Hob The group means for the second factor are equal
* H0c There is no interaction between the two factors
...
If you forget to leave out one of your dummy variables:
...
multi_regression2 <- function(x){
  x$deviceCategory <- as.factor(x$deviceCategory)
