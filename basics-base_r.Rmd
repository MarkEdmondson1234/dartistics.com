...
# Apply the function to a list of values: 1 through 3. This actually returns a list,
# which we'll cover later, and we're assigning that to an object called 'foo' that
# we're not actually doing anything with.
foo <- sapply(c(1:3), cubeValue)
```
