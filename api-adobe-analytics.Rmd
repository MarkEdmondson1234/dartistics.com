...
A best practice is to _not_ put any keys or tokens directly in your code. Rather, you can save these values to a `.Renviron` file in your home directory, and then call them via the commands below. The BIG benefit of this approach is that it reduces the chance that you will accidentally put your API keys put on Github. It's great to share your code, but not great to share access to your data!
...
dir.create("adobecache", showWarnings = FALSE)

if(!file.exists("adobecache/overtime.rds")){
  overtime.data <- QueueOvertime(reportsuite.id, 
                                           ...
                                           expedite = expedite)
  saveRDS(overtime.data, file = "adobecache/overtime.rds")
} else {
  overtime.data <- readRDS("adobecache/overtime.rds")
}
...
```{r queueranked, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
metrics <- c("visits","uniquevisitors","pageviews","event1")
elements <- c("page","geoCountry","geoCity")

if(!file.exists("adobecache/ranked.data")){
  ranked.data <- QueueRanked(reportsuite.id, date.from, date.to, metrics, elements)
  saveRDS(ranked.data, file = "adobecache/ranked.data")
} else {
  ranked.data <- readRDS("adobecache/ranked.data")  
}
...
```{r queueranked optional, message=FALSE, warning=FALSE, include=FALSE, results='hide'}
...
if(!file.exists("adobecache/ranked.data2.rds")){
  ranked.data <- QueueRanked(reportsuite.id, 
                           ...
                           data.current = data.current,
                           expedite = expedite)
  saveRDS(ranked.data, file = "adobecache/ranked.data2.rds")
} else {
  ranked.data <- readRDS("adobecache/ranked.data2.rds")
}
...
```{r queuetrended, message=FALSE, include=FALSE, results='hide'}
metrics <- c("visits","uniquevisitors","pageviews","event1")
elements <- c("page","geoCountry","geoCity")

cache_file <- "adobecache/trended.data.rds"

if(!file.exists(cache_file)){
  trended.data <- QueueTrended(reportsuite.id, date.from, date.to, metrics, elements)
  saveRDS(trended.data, file = cache_file)
} else {
  trended.data <- readRDS(cache_file)
}
...
```{r queuetrended optional, message=FALSE, include=FALSE, results='hide'}
metrics <- c("visits","uniquevisitors","pageviews","event1")
elements <- c("page","geoCountry","geoCity")
top <- 100
start <- 100
...
cache_file <- "adobecache/trended_option.rds"
...
```{r fallout, message=FALSE, include=FALSE, results='hide'}
metrics <- "pageviews"
element <- "page"
...
cache_file <- "adobecache/fallout.data.rds"

if(!file.exists(cache_file)){
  fallout.data <- QueueFallout(reportsuite.id, 
                               date.from, date.to, 
                               metrics, 
                               element, 
                               checkpoints)
  
  saveRDS(fallout.data, file = cache_file)
} else {
  fallout.data<- readRDS(cache_file)
}
...
cache_file <- "adobecache/queueexample.rds"
...
if(!file.exists(cache_file)){
  ...
  saveRDS(overtime.data, file = cache_file)
} else {
  overtime.data <- readRDS(cache_file)
}
...
```{r base_settings, echo=FALSE, eval=TRUE, error=TRUE,message=FALSE, results='hide'}
...
key <- Sys.getenv("ADOBE_API_USERNAME_M")
secret <- Sys.getenv("ADOBE_API_SECRET_M")

SCAuth(key, secret)

```

```{r web_data, message=FALSE, include=FALSE, results='hide'}
...
cache_file <- "adobecache/example.queue.trend.rds"
...
if(!file.exists(cache_file)){
  web_data <- QueueTrended(reportsuite.id,
                         ...
                         elements,
                         top = top)
  saveRDS(web_data, file = cache_file)
} else {
  web_data <- readRDS(cache_file)
}
