...
But its worth knowing how to do this without relying on a package if possible.
...
Note that if you subset `lists()` or `data.frames()` with `[ ]` it will be default return a list or data.frame back.  
...
Character subsetting is how the lookup examples work, the only different being that this time its not a `data.frame` but a `vector` that is named using the `names()` function.
...
We can then pass this into mycars row selector
...
Critical to using good data analysis so a good excercise here.