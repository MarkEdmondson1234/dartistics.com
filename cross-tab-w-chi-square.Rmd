...
sessions~o~ - sessions~e = 25 - 26.67 = -1.67
...
2. The **alpha level** ($\alpha$) we wish to use. This is the "significance level" that you choose, which is the probability of making the wrong decision when the null hypothesis (_H~0~_) is actually true. The smaller the $\alpha$ you use, the harder it is to achieve significance, but the less likely you are to draw the wrong conclusion from the data. `0.05` is a pretty common $\alpha$ level to use, but there is nothing particularly magical about that value. As a matter of fact, let's get wild and crazy and use an $\alpha$ of `0.1` for this example!

_**Aside:** You likely will never need to expose the $\chi$^2^ critical value, but, you could by using the base R function `qchisq()`. There's a quirk that we're not going to get into here to use it, in that you input `1-`$\alpha$ rather than just $\alpha$. For this example, we're using an $\alpha$ of `0.1` and have 6 degrees of freedom, so we could use `qchisq(1-0.1, 6)`, which returns **`r qchisq(1-0.1, 6) %>% round(2)`**._

With an $\alpha$ of .1 and 6 degrees of freedom, the chi square critical value is 10.64. 

We take this value and compare it to our actual $\chi$^2^ value (30.98). Since it _exceeds_ the critical chi square value (10.64), we _reject_ our hypothesis. Does that seem like it's going to be hard to keep straight? It is! So, hold on to your pants as we come at this another way.

**What we just did:** We used the _degrees of freedom_ and our _selected $\alpha$_ to calculate a _critical $\chi$^2^ value_ that we compared to the _actual $\chi$^2^ value_.
...
Does that make sense? In many cases, it is easier to simply have a statistical package return a _p-value_, and that value can then be compared to the selected $/alpha$ value.
...
* **X-squared** -- this is the "actual $\chi$^2^ value" described above. And -- _voila!_ -- it got a matching result!
